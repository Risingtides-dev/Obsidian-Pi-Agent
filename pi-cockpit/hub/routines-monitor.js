...
import { getLaunchdStatus, reloadLaunchd, readLogTail } from "./lib/launchd-utils.js";
...
# Invoke pi non-interactively (stdin pipe to pi -p)
...
`;
  ...
  reloadLaunchd(label, cfg.enabled);
...
export function deleteRoutine(slug) {
  const label = LABEL_PREFIX + slug;
  reloadLaunchd(label, false); // unload only
...
export function toggleRoutine(slug, enabled) {
  ...
  reloadLaunchd(LABEL_PREFIX + slug, cfg.enabled);
...
export function readRoutineLog(slug, lines = 50) {
  ...
  return readLogTail(logPath, lines);
}
