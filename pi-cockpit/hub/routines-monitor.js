...
/**
 * Parse a schedule string into a launchd interval/calendar config.
 * Supported:
 *   "hourly"               → StartInterval: 3600
 *   "daily@09:00"          → StartCalendarInterval: { Hour: 9, Minute: 0 }
 *   "weekdays@09:00"       → array of 5 StartCalendarIntervals (Mon-Fri)
 *   "weekly@1:09:00"       → StartCalendarInterval: { Weekday: 1, Hour: 9, Minute: 0 }
 *   "every@<minutes>"      → StartInterval: minutes*60
 *   "manual"               → null (no scheduler trigger; only Run Now)
 */
...
function buildPlistXml({ label, scriptPath, schedule, folder, logPath }) {
  ...
  // No schedule = manual; plist is still written but won't auto-run.
...
function buildRunnerScript({ folder, skillPath, logPath }) {
  // Runs the routine prompt through `pi` with the configured working dir.
  return `#!/bin/bash
set -e
cd "${folder}"
echo "[routine] $(date -Iseconds) starting" >> "${logPath}"
# Read the prompt body (everything after the second --- frontmatter marker)
PROMPT=$(awk 'BEGIN{m=0} /^---$/{m++; next} m==2{print}' "${skillPath}")
if [ -z "$PROMPT" ]; then
  echo "[routine] empty prompt, skipping" >> "${logPath}"
  exit 0
fi
# Invoke pi non-interactively with the prompt
pi run "$PROMPT" >> "${logPath}" 2>&1 || echo "[routine] pi exited $?" >> "${logPath}"
echo "[routine] $(date -Iseconds) done" >> "${logPath}"
`;
}
...
export function saveRoutine(spec) {
  // spec: { name, description, prompt, schedule, folder, enabled }
...
export function runRoutineNow(slug) {
  ...
  try {
    // Fire-and-forget; output goes to the routine's run.log
